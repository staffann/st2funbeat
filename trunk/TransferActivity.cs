...
namespace Janohl.ST2Funbeat
{
    public class TransferActivity : IAction
    {
        ...
        {
            Janohl.Funbeat.FunbeatService funbeatService = new Janohl.Funbeat.FunbeatService(Plugin.PluginSettings.User.Username, Plugin.PluginSettings.User.Password);
            foreach (IActivity activity in activities)
            {
                if (activity.Metadata.Source.IndexOf("Funbeated") < 0)
                {
                    try
                    {
                        int funbeatActivityTypeID = Plugin.PluginSettings.GetFunbeatActivityTypeID(activity.Category);
